...
\rm{DIOtGradingCoeff = m\times (1+tm1\times dt+tm2\times dt^2)}\nonumber\\
%\rm{fact2 = \frac{T}{300.15}} \nonumber\\
\rm{egfet = 1.16-\frac{7.02\times 10^{-4}\times T^2}{T+1108}} \nonumber\\
%\rm{arg = -\frac{egfet}{2kT} + \frac{1.1150877}{2\times 300.15}}\nonumber\\
\rm{pbfact = -2vt\times\left(1.5\log\left(\frac{T}{300.15}\right)+q\times \left(-\frac{egfet}{2kT} + \frac{1.1150877}{2k\times 300.15}\right)\right)}\nonumber\\
\rm{egfet1 = 1.16-\frac{7.02\times 10^{-4}\times tnom^2}{tnom+1108}}\nonumber\\
%\rm{arg1 = -\frac{egfet1}{2kT} + \frac{1.1150877}{2\times 300.15}}\nonumber\\
%\rm{fact1 = \frac{tnom}{300.15}} \nonumber\\
\rm{pbfact1 = -2vtnom\times\left(1.5\log\left(\frac{tnom}{300.15}\right)+q\times \left(-\frac{egfet1}{2kT} + \frac{1.1150877}{2\times 300.15}\right)\right)}\nonumber 
\end{eqnarray}

{\color{darkblue}\textbf{Junction capacitance variables :}}

\textbf{if tlevc is equal to 0 then}
{\color{darkgray}
\begin{eqnarray}
%\rm{} \nonumber \\
\rm{pbo = \frac{vj-pbfact1}{\frac{tnom}{300.15}}}\nonumber \\
\rm{gmaold = \frac{vj-pbo}{pbo}}\nonumber \\
\rm{tJctCap = \frac{cj}{1+tGradingCoeff\times(400\times 10^{-6}\times (tnom-300.15)-gmaold)}}\nonumber \\
\rm{tJctPot = pbfact+\frac{T}{300.15}\times pbo}\nonumber \\
\rm{gmanew = \frac{tJctPot-pbo}{pbo}}\nonumber \\
\rm{tJctCap=  }\hspace{4.5in}\nonumber \\
\rm{tJctCap\times\left(1+tGradingCoeff\times
...
{\color{darkgray}
\begin{eqnarray}
\rm{tJctCap = cj\times cta\times (T-300.15)}\nonumber \\
\nonumber 
\end{eqnarray}
}
\textbf{if tlevc is equal to 0 then}
...
\begin{eqnarray}
%\rm{}} \nonumber \\
\rm{pbosw = \frac{vjsw-pbfact1}{\frac{tnom}{300.15}}}\nonumber \\
\rm{gmaold = \frac{vjsw-pbosw}{pbosw}}\nonumber \\
\rm{tJctSWCap = \frac{cjsw}{1+tGradingCoeff\times(400\times 10^{-6}(tnom-300.15)-gmaold)}}\nonumber \\
\rm{tJctSWPot = pbfact+\frac{T}{300.15}\times pbosw}\nonumber \\
\rm{gmanew = \frac{tJctSWPot-pbosw}{pbosw}}\nonumber \\
\rm{tJctSWCap = }\hspace{4.5in}\nonumber \\
\rm{tJctSWCap(1+tGradingCoeff\times
...
\begin{eqnarray}
%\rm{} \nonumber \\
\rm{tJctSWCap = cjsw\times cta(T-300.15)}\nonumber
\end{eqnarray}
}

{\color{darkblue}\textbf{Saturation current variables :}}
\begin{eqnarray}
\rm{tSatCur = js\times \exp\left(
     \left(\frac{T}{tnom}-1\right)\frac{eg}{n\times vt}
     +\frac{xti}{n}\log\left(\frac{T}{tnom}\right) \right)}\nonumber \\
\rm{tSatSWCur = jsw\times \exp\left(
     \left(\frac{T}{tnom}-1\right)\frac{eg}{n\times vt}
     +\frac{xti}{n}\log\left(\frac{T}{tnom}\right) \right)}\nonumber										
\end{eqnarray}

{\color{darkblue}\textbf{Additional variables :}}

\begin{eqnarray}
\rm{tF1=tJctPot\times
                    \frac{1-exp((1-tGradingCoeff)\times \log\left(1- fc\right))}{1-tGradingCoeff}
										}\nonumber \\		
\rm{tF2=exp((1+tGradingCoeff)\times \log\left(1- fc\right))}\nonumber \\
\rm{tF3=1-fc\times(1+tGradingCoeff)}\nonumber \\
\rm{tF2SW=exp((1+mjsw)\times \log\left(1- fcs\right))}\nonumber \\
\rm{tF3SW=1-fcs\times(1+mjsw)}\nonumber 																										
\end{eqnarray}

{\color{darkblue}\textbf{Depletion capacitance variables :}}

\begin{eqnarray}
\rm{tDepCap=tJctPot\times fc}\nonumber 								
\end{eqnarray}

{\color{darkblue}\textbf{Vcrit :}}

\begin{eqnarray}
\rm{vte = n\times vt}\nonumber \\		
\rm{tVcrit=vte\times \log\left(\frac{vte}{(\sqrt{2}\times tSatCur\times area)}\right)}\nonumber 						
\end{eqnarray}

{\color{darkblue}\textbf{Transit time :}}

\begin{eqnarray}
\rm{tTransitTime=tt\times \left( 1+TTT1\times dt + TTT2\times dt^2 \right)}\nonumber 								
\end{eqnarray}

{\color{darkblue}\textbf{Series resistance :}}

\begin{eqnarray}
\rm{tConductance=\frac{1}{rs}\times \frac{1}{1+trs\times dt+trs2\times dt^2}}\nonumber 								
\end{eqnarray}


\textbf{\textit{Current-Voltage Terms:}}

{\color{darkblue}\textbf{ DC Current :}}
...
\begin{array}{l l}
	...
	\left\{
	\begin{array}{l l}	
	\rm{csat\times(\exp(vd/vte)-1) + gmin\times vd\nonumber\hspace{0.1in}(ikf\leq 0)} \\
	\rm{\frac{csat\times(\exp(vd/vte)-1)}{1+\sqrt{\frac{csat\times(\exp(vd/vte)-1)}{ikf\times area\times m}}} + gmin\times vd\nonumber \hspace{0.5in}(ikf>0)}
	\end{array}
	\right.\nonumber \hspace{0.2in} \rm{(vd\geq-3\times vte)}\\

	\left\{
	\begin{array}{l l}	
	\rm{-csat\times\left(1+\left(\frac{3\times vte}{e\times vd}\right)^3\right) + gmin\times vd\nonumber \hspace{0.2in}(ikr\leq 0)} \\
	\rm{\frac{-csat\times\left(1+\left(\frac{3\times vte}{e\times vd}\right)^3\right)}{1+\sqrt{\frac{-csat\times\left(1+\left(\frac{3\times vte}{e\times vd}\right)^3\right)}{-ikr\times area\times m}}} + gmin\times vd\nonumber \hspace{0.4in}(ikr>0)}
	\end{array}
	\right.\nonumber \hspace{0.2in} \rm{(-BV \geq vd<-3\times vte)}\\
		
	\left\{
	\begin{array}{l l}	
	\rm{-csat\times\exp(-\frac{BV+vd}{vte}) + gmin\times vd\nonumber\hspace{0.4in}(ikr\leq 0)} \\
	\rm{\frac{-csat\times\exp(-\frac{BV+vd}{vte})}{1+\sqrt{\frac{-csat\times\exp(-\frac{BV+vd}{vte})}{ikr\times area\times m}}} + gmin\times vd\nonumber \hspace{0.45in}(ikr>0)}
	\end{array}
